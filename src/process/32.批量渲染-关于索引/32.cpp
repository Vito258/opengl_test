...
int main() {
...

    {
        ...
        auto q2 = CreateQuad(0.1f, -0.2f, 0.0f);
        auto q3 = CreateQuad(0.3f, -0.2f, 0.0f);
        auto q4 = CreateQuad(0.5f, -0.2f, 0.0f);
        auto q5 = CreateQuad(0.7f, -0.2f, 1.0f);

        Vertex vertices[20];
        memcpy(vertices, q1.data(), q1.size() * sizeof(Vertex));
        memcpy(vertices + q1.size(), q2.data(), q2.size() * sizeof(Vertex));
        memcpy(vertices + q1.size()+ q2.size(), q3.data(), q3.size() * sizeof(Vertex));
        memcpy(vertices + q1.size()+ q2.size()+ q3.size(), q4.data(), q4.size() * sizeof(Vertex));
        memcpy(vertices + q1.size()+ q2.size()+ q3.size()+ q4.size(), q5.data(), q5.size() * sizeof(Vertex));

        //for 循环创建索引数据，假设要绘制的图形都是四边形
        // 顶点的数量
        unsigned int vertices_count = sizeof (vertices)/sizeof (vertices[0]);
        // 四边形的数量
        size_t indice_count = vertices_count / 4;
        size_t indice_size = indice_count * 6;    //  vertices/4 是四边形的个数，再*6 是绘制时需要的索引的个数

        unsigned int indices[indice_size];
        unsigned int index = 0; // 当前索引的位置
        for(unsigned int i=0;i < indice_count; i++){
            indices[index++] = i * 4 + 0; // 第一个顶点
            indices[index++] = i * 4 + 1; // 第二个顶点
            indices[index++] = i * 4 + 2; // 第三个顶点
            indices[index++] = i * 4 + 2; // 第三个顶点
            indices[index++] = i * 4 + 3; // 第四个顶点
            indices[index++] = i * 4 + 0; // 第一个顶点
        }
...
//        VertexBuffer vb(vetices, 8 * 9 * sizeof(float));
        ...
        //索引缓冲区的设置
        IndexBuffer ib(indices, indice_size);
